...
def nagtiveMean(ser):
    ...
    for i in ser:
        if i<0:
            sumNagtive += i
            numNagtive += 1
    return sumNagtive/numNagtive

def sc_gnegtiveMean(df):
    df = 1/df
    serMean = df.apply(nagtiveMean,axis=1)
    return serMean
'''过零点率'''
def countNumCrossZero(ser):
    numCrossZero = 0
    positive = True
    prePositive = True
    for i in ser:
        if i>0:
            ...
            prePositive = positive
    return numCrossZero
def sc_rateCrossZero(df):
    serNumCrossZero = df.apply(countNumCrossZero,axis=1)  
    return serNumCrossZero/63.0  #每秒钟的过零点率 8064/128 
...
if __name__ == '__main__':
    ...
    if True :
        ...
        '''记录的是电阻单位Ohm（欧姆）,这个是电导的均值'''
        sc_gmean = pd.DataFrame(sc_gmean(all_df_GSR_x),columns=['sc_gmean'])
        '''电导为负数的平均值'''
        sc_gnegtiveMean = pd.DataFrame(sc_gnegtiveMean(all_df_GSR_x),columns=['sc_gnegtiveMean'])
        '''过零点个数'''
        sc_rateCrossZero = pd.DataFrame(sc_rateCrossZero(all_df_GSR_x),columns=['sc_rateCrossZero'])
        ...
        feature_list = ['sc_mean','sc_gmean','sc_gnegtiveMean','sc_rateCrossZero','sc_median','sc_std','sc_min','sc_max','sc_range',
